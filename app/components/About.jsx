       ...
       className='flex w-full flex-col lg:flex-row items-center gap-20 my-20'>
        ...
        className='flex-1'>
          <p className='mb-10 max-w-2xl font-Ovo'>
            Computer Science student focused on DevSecOps, automation, and cloud-based software delivery. Background in Full-Stack development with React and modern web technologies, with growing experience in CI/CD pipelines, MLOps, and AI-driven operations. Interested in building secure, scalable, and production-ready systems.
          </p>
          <motion.ul
          initial={{opacity:0}}
          whileInView={{opacity:1}}
          ...
          className='flex items-center flex-wrap gap-3 sm:gap-5'>
            {toolsData.map((tool, index) => (
              ...
              className='flex items-center justify-center w-12 sm:w-14 aspect-square border border-gray-400 rounded-lg cursor-pointer hover:-translate-y-1 duration-500'
                key={index}>
                <Image src={tool} alt='Tool' className={`w-5 sm:w-7 ${tool === assets.github ? 'dark:invert' : ''}`} />
              </motion.li>
            ))}
          </motion.ul>
        </motion.div>
